...
(function(win, doc) {
    ...
    */
    var username = prompt( 'Qual o teu nome? ' );
    if( username )
    ...
    */
    var email = prompt( 'Qual o teu e-mail? ' );
    /*
    ...
    */
    $button.addEventListener( 'click', function( event ) {
        ...
        event.preventDefault(); //Se nao colucarmos ele continua a submeter o formulario

        if( !$inputUsername.value )
            return alert( 'Preencha o nome de utilizador!' );
        if( !$inputEmail.value )
            return alert( 'Preencha o email!' );
        if( !isValidEmail( $inputEmail.value ) )
            return alert( 'Introdza com um email válido!' );
        if( !$message.value )
    ...
    Alguns e-mails inválidos:
        ...
        - "agua_@evida.br.com"
    */
    function isValidEmail( email ) {  //FUNÇÕES COMEÇADAS POR is, DEVEM RETURNAR TRUE OR FALSE
